...
- The OpenAI ChatGPT OAuth adapter uses fixed OpenAI public client metadata with the OpenAI device authorization flow, then sends the resulting access token to the OpenAI-compatible API as `Authorization: Bearer ...`.
